...
## 4. Data Layer & Networking

### Networking with `Return<T>`
API 응답은 `Return<T>` 봉인 인터페이스(Sealed Interface)로 래핑하여 에러 처리를 강제합니다.
...
sealed interface Return<out T> {
  ...
  class ReturnException(throwable: Throwable): Exception(throwable), Return<Nothing>
}

inline fun <T> Return<T>.onSuccess(block: (T) -> Unit): Return<T> {
  if (this is Success) {
    block(data)
  }
  return this
...
inline fun <T> Return<T>.onHttpError(block: (HttpError) -> Unit): Return<T> {
...
inline fun <T> Return<T>.onException(block: (ReturnException) -> Unit): Return<T> {
  if (this is ReturnException) {
    block(this)
  }
  return this
}

inline fun <T> Return<T>.onComplete(block: Return<T>.() -> Unit): Return<T> {
  block(this)
  return this
}

fun <T> Return<T>.getOrDefault(default: T): T {
  return if (this is Success) data else default
}

inline fun <T, U> Return<T>.map(block: (T) -> U): Return<U> {
  return when (this) {
    is Success -> {
      Success(block(data))
    }
